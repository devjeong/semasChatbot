...
3. 인증 성공 후 "✅ 인증이 완료되었습니다. 환영합니다!" 메시지 확인
...
├─────────────────────────────────────────────────────────┤
│ 🤖 소진공: ✅ 인증이 완료되었습니다. 환영합니다!          │
│                                                         │
│ 💬 나: 이 함수를 개선해주세요                           │
│                                                         │
│ 🤖 소진공: 다음과 같이 개선할 수 있습니다...            │ ← 대화 영역
│                                                         │
│ 💬 나: 좋네요! 적용해주세요                             │
│                                                         │
│ 🤖 소진공: diff 창에서 확인해주세요                     │
...
**상황**: 새로운 환경에서 챗봇을 처음 사용하거나 서버 주소를 변경해야 한다.
...
**🌐 서버 URL 변경:**
1. **🌐 URL** 버튼 클릭
2. 새로운 서버 주소 입력 (예: `http://192.168.10.50:1234/v1`)
3. 유효성 검증 통과 후 즉시 적용
4. "LmStudio URL이 변경되었습니다" 메시지 확인

**🔄 인증 재설정:**
- **Reset** 버튼 클릭 → 인증 상태 초기화 → 재인증 팝업
- **🔐 인증** 버튼 클릭 → 수동 재인증
...
**해결법**:
1. 현재 설정된 인증키: `semas1@3`
2. 관리자에게 문의하여 새로운 인증키 발급
3. `config.properties` 파일에서 확인 가능

### ❓ 인증에 3번 실패했어요

**원인**: 최대 시도 횟수 초과
**해결법**:
1. **Reset** 버튼 클릭하여 인증 상태 초기화
2. 다시 올바른 인증키 입력
3. 그래도 안 되면 플러그인 재시작

### ❓ "❌ 인증이 필요합니다" 메시지가 계속 나와요

**원인**: 인증 세션이 끊어짐
**해결법**:
1. **🔐 인증** 버튼 클릭하여 재인증
2. 인증키 `semas1@3` 입력
3. 문제 지속 시 **Reset** 후 재시도
...
3. **🌐 URL** 버튼으로 서버 주소 확인
4. **🔐 인증** 버튼으로 인증 상태 확인
5. **🔄 Reset** 버튼 눌러 재인증 체험

### 🏃‍♂️ 초급 미션 (5분)
1. 인증 완료 확인
2. 간단한 Java 메서드 하나 선택
3. AI에게 코드 설명 요청
4. 주석 추가 요청
5. Diff 창에서 적용
...
*Version: 0.1.0* - 인증 시스템 & URL 설정 기능 추가 